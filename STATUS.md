...
- [ ] With 200 ms polling the garbage `absoluteReading` (~1e-38) became frequent: the map discarded 267 readings, 77/121 points failed all 3 tries (NaN) and the map took 19.5 s. `relativeReading` stays plausible in those readings → consider deriving the signal from relative × range full scale instead of absolute
